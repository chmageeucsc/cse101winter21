...
void printList(FILE* out, List L) {
	...
	Node temp = L->front;
	while(temp != NULL) {
		fprintf(out, "%ld", temp->data);		// fix to print strings later
		temp = temp->next;
		if(temp != NULL) {
			fprintf(out, " ");
		}
...
List copyList(List L) {	
	...
	LIST_ELEMENT indexHold = L->index;		// keeps track of L's index
	moveFront(L);
	while(L->cursor != NULL) {
...
List concatList(List A, List B) {
	...
	LIST_ELEMENT indexHold = B->index;		// keeps track of L's index
	moveFront(B);
	while(B->cursor != NULL) {
